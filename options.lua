...
local function Parse(arg)
    ...
    cmd:text()
    cmd:option('-expID',        'stacked-aes-ensemble', 'Experiment ID')
    cmd:option('-ensembleID',   'hg-generic', 'Experiment ID')
    cmd:option('-dataset',        'lsp+mpii', 'Dataset choice: mpii | flic | lsp | coco | lsp+mpii')
    ...
    cmd:text()
    cmd:option('-netType',  'stacked-aes-ensemble', 'Options: hg-stacked | hg-generic | hg-generic-rnn | hg-generic-wide | hg-generic-deconv | ' ..
    ...
    cmd:option('-nonlinearity',   'relu', 'Applied nonlinearity: relu, elu, prelu, rrelu or leakyrelu.')
    ...
    cmd:text()
    cmd:option('-trainIters',      5000, 'Number of train batches/iterations per epoch')
    cmd:option('-testIters',        500, 'Number of test batches/iterations used for accuracy validation per epoch')
    cmd:option('-nEpochs',           90, 'Total number of epochs to run')
    cmd:option('-batchSize',          4, 'Mini-batch size')
    cmd:option('-schedule', "{{50,2.5e-4,0},{25,1e-4,0},{15,5e-5,0}}", 'Optimization schedule. Overrides the previous configs if not empty.')
    cmd:text()
    cmd:text(' ---------- Data options ---------------------------------------')
    cmd:text()
    cmd:option('-inputRes',          256, 'Input image resolution')
    cmd:option('-outputRes',          64, 'Output heatmap resolution')
    cmd:option('-scale',             .30, 'Degree of scale augmentation')
    cmd:option('-rotate',             40, 'Degree of rotation augmentation')
    cmd:option('-hmGauss',             1, 'Heatmap gaussian size')
    cmd:text()
    cmd:text(' ---------- Data augments ---------------------------------------')
    cmd:text()
    cmd:option('-colourNorm',     "false", 'mean/std norm')
    cmd:option('-colourjit',      "true", 'colour jittering (other method)')
    ...
    cmd:option('-rotRate',             .2, 'Rotation probability.')
    cmd:text()
    cmd:text(' ---------- Accuracy options -----------------------------------')
    cmd:text()
    cmd:option('-subpixel_precision',   "true", 'Compute parts x,y coordinate using sub-pixel precision (parabola fitting).')
    cmd:text()
