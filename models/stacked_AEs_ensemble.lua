...
local function createModel()
    ...
    for i = 1,opt.nStack do
        ...
        --local ll = lin(opt.nFeats,512,spDropout_ll)
        ...
        --local tmpOut = nn.SpatialConvolution(512,outputDim[1][1],1,1,1,1,0,0)(spDropout_Out)

        if i > 1 then
            local concat = nn.JoinTable(2)({tmpOut,out[#out]})
            local tmpOut2 = nn.ReLU(true)(nn.SpatialConvolution(outputDim[1][1]*2,outputDim[1][1],1,1,1,1,0,0)(concat))
    ...
    local out_final = nn.ReLU(true)(lin(512, outputDim[1][1], ll1))

    table.insert(out,out_final)
