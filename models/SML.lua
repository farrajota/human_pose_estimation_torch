...
local function beautify_modules(modules)
    ...
    -- upsampling layers
    if modules.upsample then
        for k, upsample_layer in pairs(modules.upsample) do
            upsample_layer:annotate{
                name = 'UPSampling layer',
                description = 'Output heatmaps (256x256)',
                graphAttributes = {
                    color = 'black',
                    style = 'filled',
                    fillcolor = 'green'}
...
local function createModel()
    ...
    local cat_m_out = nn.JoinTable(2)({out_s,out_m})
    local out_m_2 = nn.SpatialConvolution(outputDim[1][1]*2,outputDim[1][1],1,1,1,1,0,0)(cat_m_out)
    local out_m_ = nn.SpatialConvolution(outputDim[1][1],512,1,1,1,1,0,0)(out_m_2)
    ...
    local cat_l_out = nn.JoinTable(2)({out_m_2,out_l})
