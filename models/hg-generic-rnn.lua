...
local function createModel()
    ...
    local hg = hourglass(4,opt.nFeats,input)
    -- Linear layer to produce first set of predictions
    local ll = lin(opt.nFeats,opt.nFeats,hg)
    local stack = nn.gModule({input},{ll})
    
    
    for i = 1,opt.nStack do
        local hg = stack:clone('weight','bias','running_mean','running_var','gradWeight','gradBias')(inter)
        --local hg = stack:clone('weight')(inter)
