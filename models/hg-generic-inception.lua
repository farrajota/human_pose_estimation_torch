paths.dofile('layers/Inception.lua')

local function hourglass(n, f, inp)
    -- Upper branch
    local up1 = InceptionResidual(f,f)(inp)

    -- Lower branch
    local pool = nn.SpatialMaxPooling(2,2,2,2)(inp)
    local low1 = InceptionResidual(f,f)(pool)
    local low2

    if n > 1 then low2 = hourglass(n-1,f,low1)
    else low2 = InceptionResidual(f,f)(low1) end

    local low3 = InceptionResidual(f,f)(low2)
...
local function createModel()
    ...
    local r1 = InceptionResidual(64,128)(cnv1)
    local pool = nn.SpatialMaxPooling(2,2,2,2)(r1)                       -- 64
    local r4 = InceptionResidual(128,128)(pool)
    local r5 = InceptionResidual(128,opt.nFeats)(r4)
