...
local function createModel()
    ...
    local out_s_ = nn.SpatialConvolution(outputDim[1][1],256+256,1,1,1,1,0,0)(out_s)

    -- Concatenate with previous linear features
    local cat1 = nn.JoinTable(2)({l2_s,r4})
    local cat1_ = nn.SpatialConvolution(256+256,256+256,1,1,1,1,0,0)(cat1)
    local int1 = nn.CAddTable()({cat1_,out_s_})

    -- increase resolution for the second network
    local r5 = Residual(256+256,256+256, 'U')(int1)
    ...
    local autoencoder_m = AE_medium(5, 256+256, 512)  -- medium AE
    ...
    local out_m_ = nn.SpatialConvolution(outputDim[1][1],512+256,1,1,1,1,0,0)(out_m)

    -- Concatenate with previous linear features
    local upsample_r4 = nn.SpatialUpSamplingNearest(2)(r4)
    local cat2 = nn.JoinTable(2)({l2_m,upsample_r4})
    local cat2_ = nn.SpatialConvolution(512+256,512+256,1,1,1,1,0,0)(cat2)
    local int2 = nn.CAddTable()({cat2_,out_m_})

    -- increase resolution for the third network
    local r6 = Residual(512+256,512+256, 'U')(int2)

    --------------------------------------------------------------------------------
    -- Large Auto-Encoder Network
    --------------------------------------------------------------------------------

    local autoencoder_l = AE_large(6, 512+256)  -- large AE
    local autoencoder_l_out = autoencoder_l(r6)
    ...
    local model = nn.gModule({input}, {out_s_up, out_m_up, out_l})
