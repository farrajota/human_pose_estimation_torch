...
local function hourglass(n, k, f, depth, inp)
    -- Upper branch
    ...
    if n > 1 then low2 = hourglass(n-1,k, f, depth,low1)
...
local function createModel()
    ...
    local cnv2_ = nn.SpatialConvolution(64,64+16*widening,1,1,1,1)(cnv1)           -- 128
    local cnv2 = nn.ReLU(true)(nn.SpatialBatchNormalization(64+16*widening)(cnv2_))
    local r1 = WideResidual(64+16*widening,128+16*widening, depth, 2)(cnv2)   -- 64     -- tinha 2 em vez de 1
    local r4 = WideResidual(128+16*widening,128+16*widening, depth, 1)(r1)
    local r5 = WideResidual(128+16*widening,opt.nFeats+16*widening, depth, 1)(r4)

    local out = {}
    local inter = r5

    for i = 1,opt.nStack do
        local hg = hourglass(4,widening, opt.nFeats+16*widening, depth,inter)

        -- Linear layer to produce first set of predictions
        local ll = lin(opt.nFeats+16*widening,opt.nFeats+16*widening,hg)

        -- Predicted heatmaps
        local tmpOut = nn.SpatialConvolution(opt.nFeats+16*widening,outputDim[1][1],1,1,1,1,0,0)(ll)
